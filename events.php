...
<html lang="en">
  ...
  <section class="events-section">
    <h2>🎉 Upcoming Events in Nkomazi</h2>
    <p>Stay updated on what’s happening in your area.</p>

    <!-- Event search -->
    <div class="event-search">
      <input type="text" id="eventSearchInput" placeholder="Search events...">
      <button id="eventSearchBtn">Search</button>
    </div>
	...
	if ($result->num_rows > 0) {
		...
		if (!empty($row['image'])) {
			echo "<img src='uploads/" . $row['image'] . "' alt='Event Image' style='width:275px;height:307px; border-radius:8px; margin-bottom:10px;'>";
		...
		echo "
			...
			<p>{$row['description']}</p>
			<a href='#' target='_blank' class='event-btn'><i class='fab fa-whatsapp'></i> Get Info</a>
			</div>";
			}
		} else {
			echo "<p style='text-align:center; color:#777;'>No events available at the moment.</p>";
		}
	?>
  ...
  <script>
    document.getElementById('eventSearchInput').addEventListener('keyup', function() {
